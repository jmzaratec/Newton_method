...
float a=0.,b=0.,c=0., initial=0., initial2=0.; 
...
// 1) Square
int square (float a1,float b1,float c1)
{
	float d1;
	d1 = (b1*b1) - (4*a1*c1);
	if (d1>=0)
...
// 2.1 function
float function(float a2,float b2,float c2,float x)
{
	return((a2*(x*x))+(b2*x)+ (c2));
}

// 2.2 derfunction
float derfunction(float a3,float b3,float x)
{
	return((2*a3*x)+(b3));
}

// 2.3 iteractions
float ite(float func, float der, float x)
{
	return (x-(func/der));
}
// 2.4 Newton
...
{	
	float x0old, max=100000,tol=1.e-6; 
